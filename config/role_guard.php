...
function showAccessDenied($message, $buttonText, $buttonHref) {
    ...
    <body class="bg-gray-50 flex items-center justify-center min-h-screen">
        <div class="text-center p-8 bg-white rounded-2xl shadow-lg max-w-md mx-4">
            ...
            <a href="' . htmlspecialchars($buttonHref) . '" class="inline-block px-6 py-2.5 bg-blue-600 text-white rounded-xl font-semibold hover:bg-blue-700 transition">' . htmlspecialchars($buttonText) . '</a>
        </div>
    </body>
    </html>';
    exit();
...
function checkRole($allowedRoles)
{
    ...
    if (!isset($_SESSION['id_user']) || !isset($_SESSION['role_name'])) {
        showAccessDenied('Akses Ditolak: Anda belum login. Silakan login terlebih dahulu.', 'Kembali ke Login', getBaseUrl() . 'index.php');
    }
    ...
    if (!in_array($userRole, $allowedRoles, true)) {
        showAccessDenied('Akses Ditolak: Anda tidak memiliki otoritas untuk halaman ini.', 'Kembali', 'javascript:history.back()');
    }
}
